...
def main():
    ...
    while True:
        chosen_category = input(
            "\033[36mWhich type of the workout would you like to get?\033[0m "
        )
        clear()
        if chosen_category.lower() in ["upper body", "lower body", "full body"]:
            ...
            print(
                "This category is not found. Please choose between UPPER BODY, LOWER BODY, FULL BODY, or WOD!"
            )
...
def get_streching_exercises():
    ...
    response = requests.get(
        api_url, headers={"X-Api-Key": "Tnfq58nczmtcqvtPKgkdsQ==o9PUVZvWmRWWljjS"}
    )
    if response.status_code == requests.codes.ok:
        ...
        print(
            "Oops, It seems we'are experiencing some technical difficultis right now. Please try again later."
        )
...
def main_menu():
    ...
    typing(
        "Whether you are a beginner or a seasoned fitness enthusiast, we have a workout for you!\033[0m\n"
    )
    typing("MAIN MENU: \n")
    typing("1. UPPER BODY")
    typing("2. LOWER BODY")
